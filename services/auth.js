    ...
    if (user) {
      ...
      if (password){
        ...
        if (!isPasswordMatched) {
          await dbService.updateOne(User,{ _id:user.id },{ loginRetryLimit:user.loginRetryLimit + 1 });
          return {
            flag:true,
...
const changePassword = async (params)=>{
  try {
    ...
    if (user && user.id) {
      ...
      let updatedUser = dbService.updateOne(User,{ _id:user.id },{ 'password':password });
      if (updatedUser) {
        return {
          flag:false,
